...
**자동 실행 내용**:
1. 종목 필터링 (2,790개 → 500개)
2. DB 저장 (monitoring 상태로 저장)
3. AI 분석 (Top 20 종목 자동 분석)

> **소요 시간**: 약 3~5분 (네이버 서버 응답 속도 + AI 분석 시간)
...
### 2. 승인 종목 재평가 (자동화 권장)
승인된 종목을 매일 재평가하여 탈락 기준을 체크합니다.
```bash
python3 evaluate_approved.py
```

**재평가 기준**:
- 3일 미만: 재평가 대기
- 3~7일: 매일 아래 조건 체크
  - final_score 20% 이상 하락
  - RSI > 75 AND close < MA5 (과매수 + 하락)
  - 3일 거래량 < 60일 평균 * 50%
  - Stock Pool Top 100 탈락
- 7일 이상: 자동 탈락

**AI 리포트 상태 관리**:
재평가 시 AI 리포트 상태도 자동으로 업데이트됩니다.
- ACTIVE: 현재 모니터링 중 (monitoring, approved)
- TRADED: 실제 거래 진행 중 (trading, completed)
- DROPPED: 재평가 탈락 또는 거부 (rejected)

수동으로 AI 리포트 상태를 동기화하려면:
```bash
python3 update_ai_report_status.py
```

**Cron 설정 예시** (매일 오전 9시 실행):
```bash
0 9 * * * cd /home/greatbps/projects/stockgravity && python3 evaluate_approved.py >> logs/evaluate.log 2>&1
```
